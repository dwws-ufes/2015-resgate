...
import br.com.resgate.excecoes.ConversaodeDadosException;
...
@WebServlet(urlPatterns = { "/data/Ambulancia/*" })
public class ConversaodeDadosAmbulanciaServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp)
		...
		ambulancia = ambulanciaDAO.recuperarPorPlaca(placa);
		id = Long.toString(ambulancia.getId().longValue());
		try {
			in = FileManager.get().open(
					"http://localhost:2020/data/Ambulancia/" + id);
			out = resp.getWriter();
		} catch (Exception e) {
		}
